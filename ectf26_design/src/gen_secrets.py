...
Generates five 256-bit keys with separated responsibilities:
  ...
  TRANSFER_AUTH_KEY — HMAC-SHA256 for all protocol challenge-response
                      (sender_auth, receiver_auth, interrogate, permission MAC).
  PERM_KEY          — HMAC-SHA256 for computing PERMISSION_MAC at build time.
                      This key is consumed by secrets_to_c_header.py and is
                      NOT written to firmware flash.  An attacker who dumps
                      the device never sees PERM_KEY.
  PIN_KEY           — HMAC-SHA256 for computing PIN_HMAC at build time.
                      Likewise not present in firmware flash.
...
def gen_secrets(groups: list[int]) -> bytes:
    ...
    transfer_auth_key = crypto_secrets.token_bytes(32)  # HMAC for protocol auth
    perm_key          = crypto_secrets.token_bytes(32)  # HMAC for PERMISSION_MAC (build-time only)
    pin_key           = crypto_secrets.token_bytes(32)  # HMAC for PIN_HMAC (build-time only)
