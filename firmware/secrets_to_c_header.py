...
Per-device key (generated HERE, NOT from global.secrets):
  ...
  Security consequence of the old design: a single SCA extraction of the
  shared PIN_KEY + a flash dump of any device's PIN_HMAC was sufficient to
  brute-force all other devices' PINs in seconds offline (16^6 ≈ 16.7M
  candidates at >10M HMAC/s).  Per-device PIN_KEY closes this entirely.
...
def secrets_to_c_header(
    ...
    with open(header_path, 'w') as f:
        ...
        f.write(' *\n')
        f.write(' * PIN_KEY is a per-device secret generated by secrets_to_c_header.py at\n')
        f.write(' * Build HSM time.  It does NOT appear in global.secrets so that extracting\n')
        f.write(' * it from one device cannot be used to brute-force another device\'s PIN.\n')
        f.write(' */\n\n')
