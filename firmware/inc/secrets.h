 ...
 *
 * PIN_KEY is a per-device secret generated by secrets_to_c_header.py at
 * Build HSM time.  It does NOT appear in global.secrets so that extracting
 * it from one device cannot be used to brute-force another device's PIN.
 */
...
/* HMAC-SHA256 key for PIN verification — per-device, not in global.secrets. */
