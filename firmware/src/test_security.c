...
/* Enable this to run security tests on boot */
#define RUN_SECURITY_TESTS 0
...
void init(void) {
    ...
    // Initialize hardware TRNG - CRITICAL for security functions
    int trng_result = trng_init();
    if (trng_result != 0) {
        sprintf(output_buf, "TRNG init failed: %d\n", trng_result);
        print_error(output_buf);
        // Halt on TRNG failure - security functions require randomness
        ...
        while (1) {
            ...
            // Longer pause between error code repetitions
...
#if RUN_SECURITY_TESTS
    // Run security module tests (disable for production)
    run_security_tests();
#endif
